console.log("Finsweet Extended Loaded");

function toggleResetButton() {
            const zipcodeInput = document.getElementById('zipcode').value.trim();
        ...
        // Insert the combined JavaScript code here
        "use strict";
...
(() => {
    ...
    var oe = (t, e, r, n) => (n && Yt(t), r ? !0 : e.every(o => Ur(t, o))),
        Ur = (t, {
            ...
            let c = e.filter(u => {
                ...
                if (o === "range") {
                    let [A] = g, [C, v] = a, S = re(C, A, v, T);
                    return S && s && (f == null || f.set(A, {
                        highlightCSSClass: i
                    })), S
                }
                let b = a.filter(C => {
                    if (d === "from" || d === "to") {
                        let [v, S] = g, x = re(C, v, S, T);
                        return x && s && (f == null || f.set(v, {
                            highlightCSSClass: i
                        }), f == null || f.set(S, {
                            highlightCSSClass: i
                        })), x
                    ...
                    return g.some(S => {
                        ...
                        if (T === "date" && !m) {
                            let [z, N] = [C, S].map(B => {
                                var bt;
                                return (bt = Tt(B)) == null ? void 0 : bt.getTime()
                            });
                            x = z === N
                        } else l.some(({
                            type: z
                        }) => !["checkbox", "radio", "select-one"].includes(z)) || e.length > 1 ? x = S.toLowerCase().includes(C.toLowerCase()) : x = C.toLowerCase() === S.toLowerCase();
                ...
                return n === "all" ? b.length === a.length : b.length > 0
            });
    ...
    var st = class {
        ...
        async resetFilters(e, r) {
            ...
            if (!e || !e.length)
                ...
                let o = n.find(i => q(i.filterKeys, e));
                if (!o) return;
                Et(o, r)
            }
            let s = !r;
    ...
    async function selectLocation(placeId, description) {
        placesService.getDetails({
            ...
            if (status === google.maps.places.PlacesServiceStatus.OK) {
                ...
                const latLongEl = document.querySelector('.longitude-latitude');
                latLongEl.innerHTML = `${lat},${lng}`;

                const zipcodeInput = document.getElementById('zipcode');
                zipcodeInput.value = description;

                const textField = document.getElementById('activezipcode');
                textField.textContent = description;

                clearSuggestions();
                filterAndMoveItems(lat, lng);
            }
        });
    }
    ...
    function filterAndMoveItems(baseLat, baseLng) {
        const hiddenItemsContainer = document.getElementById('all-items-hidden');
        ...
        const items = hiddenItemsContainer.querySelectorAll('[data-lat][data-lng]');
        ...
        items.forEach(item => {
            ...
            const distance = haversine(baseLat, baseLng, itemLat, itemLng);
    ...
    document.getElementById('resetButton').addEventListener('click', resetLocationItems);
    document.getElementById('zipcode').addEventListener('input', toggleResetButton);

    window.onload = function () {
        initializeAutocomplete();
    };

    // Script for show and hide of autocomplete UI
    document.addEventListener("DOMContentLoaded", function () {
        ...
        resultWrapper.addEventListener("mousedown", function (event) {
        ...
        document.querySelectorAll(".search-result-link").forEach(function (link) {
            link.addEventListener("click", function (event) {
                ...
                let selectedText = link.querySelector(".predictive-text").textContent;
    ...
    // To mirror the input from input field to active filter text field
    document.addEventListener('DOMContentLoaded', function () {
        const newinputField = document.getElementById('zipcode');
        const newtextField = document.getElementById('activezipcode');

        newinputField.addEventListener('input', function () {
            newtextField.textContent = this.value;
        });
    });
})();
